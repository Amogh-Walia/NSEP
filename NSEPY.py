...
#Here 'expiry' is the expiry-date for which we will be getting the data for 
    ...
    for i in wb.sheets:
        # i will be in the format <Sheet [nifty2.xlsm]12-Nov-2020>
        #we will be splicing the string
        sheet_list.append(str(i)[20:-1])

        
    #now we will check if there is a page in the workbook for the given date
    #If not we will add a page
...
#PASTING THE DATA IN EXCEL
        ...
        sheet.range("J3").options(index=False,header = True).value = pe_data.drop(
...
#ANNOUNCING TIME AT WHICH THIS WORK WAS DONE
        sheet.range("A1").value = Get_time()
